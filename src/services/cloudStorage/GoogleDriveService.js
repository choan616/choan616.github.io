...
class GoogleDriveService extends CloudStorageInterface {
  constructor() {
    ...
    // v1.1: 메서드 동기화 문제 해결을 위한 리빌드 트리거
  ...
  async restoreSession(email) {
    ...
    return new Promise((resolve) => {
      try {
        // 성공/실패 처리를 위한 임시 핸들러 설정
        const originalCallback = this.tokenClient.callback;

        // 일회성 콜백으로 대체하여 silent sign-in 결과 처리
        this.tokenClient.callback = (response) => {
          // 원래 콜백 복구 (다른 로그인 시도를 위해)
          this.tokenClient.callback = originalCallback;

          if (response.error) {
            console.log('Silent sign-in failed:', response.error);
            resolve(false);
            return;
          }
        ...
        // prompt: ''를 사용하여 사용자 상호작용 없이 토큰 요청
        ...
        console.error('Error during restoreSession:', err);
        resolve(false);
      }
    });
  }
  ...
  // CloudStorageInterface 준수를 위한 메서드 별칭
  async uploadBackup(zipBlob, metadata, onProgress) {
    return this.syncToGoogleDrive(zipBlob, metadata, onProgress);
  }

  async downloadBackup(fileId, onProgress) {
    return this.restoreFromGoogleDrive(fileId, onProgress);
  }

  async deleteBackup(fileId) {
    return this.deleteBackupFile(fileId);
  }
}
